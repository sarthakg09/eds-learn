...
function silhouetteMarkup() {
  return `
    <svg class="cp-silhouette" viewBox="0 0 640 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
      <ellipse cx="320" cy="248" rx="260" ry="18" fill="#000" opacity="0.18"/>
      <path class="cp-body" d="M40 190 L70 110 Q110 30 220 20 L420 20 Q510 28 555 110 L585 190 L585 220 Q585 240 565 240 L535 240 Q528 205 495 205 Q462 205 455 240 L185 240 Q178 205 145 205 Q112 205 105 240 L75 240 Q40 240 40 220 Z" fill="var(--cp-color, #d61f26)"/>
...
function buildVisualizer(car) {
  ...
  selectedLabel.className = 'cp-selected-label';

  const swatchList = document.createElement('div');
  swatchList.className = 'cp-swatches';
  swatchList.setAttribute('role', 'group');
  swatchList.setAttribute('aria-label', 'Customize Your Beast');

  const applyColor = (hex) => {
    stage.style.setProperty('--cp-color', hex);
...
export default async function decorate(block) {
  ...
  block.innerHTML = '<div class="cp-skeleton skeleton" aria-busy="true"></div>';

  try {
    const res = await fetch(endpoint);
    if (!res.ok) throw new Error(`color-picker: ${endpoint} returned ${res.status}`);
    const json = await res.json();
    const cars = Array.isArray(json.data) ? json.data : [];
    const car = (modelId && cars.find((c) => c.id === modelId)) || cars.find((c) => c.colors?.length);

    if (!car) {
      block.innerHTML = '<p class="error-state" role="alert">No color options found.</p>';
      return;
    }
    ...
    console.error('color-picker failed to load', error);
    block.innerHTML = '<p class="error-state" role="alert">We couldn\'t load color options right now.</p>';
  }
}
