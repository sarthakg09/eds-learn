 ...
 * matching the `bm-hero` model field order: image, title, subtitle,
...
function buildCta(text, href, variant) {
  ...
  a.className = `bm-hero-cta bm-hero-cta-${variant}`;
  a.href = href || '#';
  a.textContent = text;
  return a;
}
...
export default function decorate(block) {
  ...
  media.className = 'bm-hero-media';
  if (picture) media.append(picture);

  const content = document.createElement('div');
  content.className = 'bm-hero-content';
  ...
  ctaWrap.className = 'bm-hero-ctas';
  ...
  block.textContent = '';
  block.classList.add(`bm-hero-theme-${theme}`);
